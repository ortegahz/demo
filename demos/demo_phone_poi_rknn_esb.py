...
def run(args):
    PATH_SAVE_VIDEO = '/home/manu/tmp/results.mp4'  # force *.mp4 suffix on results videos

    inferer_esb = Inferer(args.path_in_mp4, False, 0, args.weights_esb, 0, args.yaml_esb, args.img_size, False)
    ...
    vid_writer = cv2.VideoWriter(PATH_SAVE_VIDEO, cv2.VideoWriter_fourcc(*'mp4v'), 5, (1920, 1080))
    ...
    while True:
        ...
        if det_people is not None:
            for *xyxy, conf, cls in reversed(det_people):
                label = f'{conf:.2f}'
                inferer_esb.plot_box_and_label(frame, max(round(sum(frame.shape) / 2 * 0.003), 2), xyxy, label,
                                               color=(255, 0, 0))
        if len(det_play):
        ...
        for idx, (*xyxy, conf_play, _) in enumerate(det_play):
            ...
            for idx_s, s in enumerate(inferer_esb.model_rknn.stride):
