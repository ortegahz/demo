...
class RetinaFace:
  def __init__(self, prefix, epoch, ctx_id=0, network='net3', nms=0.4, nocrop=False, decay4 = 0.5, vote=False):
    ...
    # pixel_means=[127.5, 127.5, 127.5]
    # pixel_stds=[128.0, 128.0, 128.0]
    ...
    #   # arguments = all_layers.list_arguments()
    #   outputs = all_layers.list_outputs()
    #   arg_shapes, out_shapes, aux_shapes = all_layers.infer_shape(data=(1, 3, 1920, 1088))
    #   for output, out_shape in zip(outputs, out_shapes):
    #     print('%s --> %s\n' % (output, out_shape))
    ...
    for img in imgs:
      for im_scale in scales:
        for flip in flips:
          ...
          # # save outputs
          # for save_i in range(len(net_out)):
          #     save_output = net_out[save_i].asnumpy().flatten()
          #     np.savetxt('/home/manu/tmp/mxnet_output_%d.txt' % save_i, save_output,
          #                fmt="%f", delimiter="\n")
